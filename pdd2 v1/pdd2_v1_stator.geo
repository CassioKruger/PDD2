...
For i In {0:N_ss-1}
	 ...
	 Point(dP+1) = {0,R_sin,0,m_normal};
	 Point(dP+2) = {Cos(Pi/2 - 30*deg2rad)*R_sin,Sin(Pi/2 - 30*deg2rad)*R_sin,0,m_normal};

	 //abertura no slot, abaixo dos enrolamentos
	 Point(dP+3) = {0, R_sin+0.002,0,m_normal};
	 Point(dP+4) = {Cos(Pi/2 - 3*deg2rad)*(R_sin+0.002), Sin(Pi/2 - 3*deg2rad)*(R_sin+0.002),0,m_normal};
	 Point(dP+5) = {Cos(Pi/2 - 3*deg2rad)*(R_sin+0.002), Sin(Pi/2 - 3*deg2rad)*(R_sin+0.002 + 0.003),0,m_normal};
	 Point(dP+6) = {0, R_sin+0.002+0.003,0,m_normal};

	 //parte dos enrolamentos
	 Point(dP+7) = {Cos(Pi/2 - 24*deg2rad)*(R_sin+0.002+ 0.003) , Sin(Pi/2 - 24*deg2rad)*(R_sin+0.002+ 0.003) ,0,m_normal};
	 Point(dP+8) = {0, R_sin+0.002+0.003+0.03,0,m_normal};
	 Point(dP+9) = {Cos(Pi/2 - 24*deg2rad)*(R_sin+0.002+0.003+0.03), Sin(Pi/2 - 24*deg2rad)*(R_sin+0.002+0.003+0.03),0,m_normal};

	 //parte superior do slot
	 Point(dP+10) = {0,R_sin+0.002+0.003+0.03+0.015,0,m_normal};
	 Point(dP+11) = {Cos(Pi/2 - 30*deg2rad)*(R_sin+0.002+0.003+0.03+0.015),Sin(Pi/2 - 30*deg2rad)*(R_sin+0.002+0.003+0.03+0.015),0,m_normal};

	 //sliding e airgap
	 Point(dP+12) = {0,R_gs,0,m_normal};
	 Point(dP+13) = {Cos(Pi/2 - 30*deg2rad)*R_gs,Sin(Pi/2 - 30*deg2rad)*R_gs,0,m_normal};
